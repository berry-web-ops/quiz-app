...
export const QuestionsCard = ({data, sendDataToParent}: {data: Question[], sendDataToParent: (quizdata: {count: number, result: boolean[]}) => void }) => {
    ...
    return (
        ...
        return (
            ...
            <CardContent>
            <Typography gutterBottom variant="h4" className='m-6 p-3em place-content-center font-bold text-center items-center self-center text-ellipsis' component="div">
            Question {quizdata.count+1}/10
            </Typography>
            <Typography variant="h5" className='place-content-center font-sans text-center items-center self-center text-gray-800 text-balance m-4 p-2em' sx={{ color: 'text.secondary' }} component="div">
                <div key={index}>{el.question}</div>
            </Typography>
            </CardContent>
            <CardActions className='flex flex-wrap justify-center gap-4 p-4'>
                    <Button variant="outlined" className='text-ellipsis text-wrap w-80 h-16 m-3 p-3 text-lg justify-around items-center' onClick={handleClick}>{shuffledArr[0]}</Button>
                    <Button variant="outlined" className='text-ellipsis text-wrap w-80 h-16 m-3 p-3 text-lg justify-around items-center' onClick={handleClick}>{shuffledArr[1]}</Button>
                    <Button variant="outlined" className='text-ellipsis text-wrap w-80 h-16 m-3 p-3 text-lg justify-around items-center' onClick={handleClick}>{shuffledArr[2]}</Button>
                    <Button variant="outlined" className='text-ellipsis text-wrap w-80 h-16 m-3 p-3 text-lg justify-around items-center' onClick={handleClick}>{shuffledArr[3]}</Button>
            </CardActions>
            </Card>
         )}
        )}
